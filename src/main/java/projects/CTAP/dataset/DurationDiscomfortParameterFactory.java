...
public class DurationDiscomfortParameterFactory implements ParameterFactoryI{

	private final List<AgentActivityLink> agentsActivities;
	private final List<List<Long>> ids;
	
	public DurationDiscomfortParameterFactory(List<AgentActivityLink> agentsActivities,List<List<Long>> ids) {
	...
	public ParameterI run() {
		...
		Map<String,Double> map = agentsActivities.stream()
				.collect(Collectors.toMap(x -> x.getAgentId().toString()+x.getActivityId().toString(),x -> x.getDurationDiscomfort()));
		int ii = 0;
		int jj = 0;
		...
		return new DurationDiscomfortParameter(parameter,ids);
	}
