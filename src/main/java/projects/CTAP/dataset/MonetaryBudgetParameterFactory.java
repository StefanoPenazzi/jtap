...
public class MonetaryBudgetParameterFactory implements ParameterFactoryI {
	
	private final List<StdAgentNodeImpl> agents;
	private final List<List<Long>> parameterDescription;
	
	public MonetaryBudgetParameterFactory(List<StdAgentNodeImpl> agents,List<List<Long>> parameterDescription) {
		this.agents = agents;
	...
	public ParameterI run() {
	    ...
	    for(Long i: parameterDescription.get(0)) {
	    	parameter[ii] = map.get(i).getMonetaryBudget();
	    	ii++;
	    }
		return new MonetaryBudgetParameter(parameter, parameterDescription );
